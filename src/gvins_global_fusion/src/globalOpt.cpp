...
void GlobalOptimization::inputOdom(double t, Eigen::Vector3d OdomP, Eigen::Quaterniond OdomQ)
{
    ...
    globalQ.normalize();  // Normalize quaternion before storing
    ...
    lastP = globalP;
    lastQ = globalQ;
    lastQ.normalize();  // Normalize quaternion to ensure unit norm
...
{
    mPoseMap.lock();
    // Look up the optimized pose from globalPoseMap for timestamp t
    // globalPoseMap contains optimized poses after GPS fusion
    map<double, vector<double>>::iterator iter = globalPoseMap.find(t);
    if (iter != globalPoseMap.end())
    {
        // Found pose for this timestamp (may be optimized or initial)
        vector<double> pose = iter->second;
        odomP = Eigen::Vector3d(pose[0], pose[1], pose[2]);
        odomQ = Eigen::Quaterniond(pose[3], pose[4], pose[5], pose[6]);
        odomQ.normalize();  // Normalize quaternion to ensure unit norm
    }
    else
    {
        // Exact timestamp not found, find closest timestamp
        if (!globalPoseMap.empty())
        {
            iter = globalPoseMap.lower_bound(t);
            if (iter == globalPoseMap.end())
            {
                // t is larger than all timestamps, use last
                iter--;
            }
            else if (iter != globalPoseMap.begin())
            {
                // Check if previous element is closer
                map<double, vector<double>>::iterator prev = iter;
                prev--;
                if (std::abs(t - prev->first) < std::abs(t - iter->first))
                    iter = prev;
            }
            vector<double> pose = iter->second;
            odomP = Eigen::Vector3d(pose[0], pose[1], pose[2]);
        ...
        {
            // Map is empty, use lastP/lastQ (initial transformation)
            odomP = lastP;
            odomQ = lastQ;
            odomQ.normalize();  // Normalize quaternion to ensure unit norm
        }
...
void GlobalOptimization::optimize()
{
    while(true)
    {
        ...
        {
            newGPS = false;
            ROS_DEBUG_THROTTLE(2.0, "gvins_global_fusion: Ceres global optimization");
            ...
            for (int i = 0; i < length; i++, iter++)
            {
            	// Normalize quaternion from optimization result before storing
            	Eigen::Quaterniond q_opt(q_array[i][0], q_array[i][1], q_array[i][2], q_array[i][3]);
            	q_opt.normalize();
            	vector<double> globalPose{t_array[i][0], t_array[i][1], t_array[i][2],
            							  q_opt.w(), q_opt.x(), q_opt.y(), q_opt.z()};
...
void GlobalOptimization::updateGlobalPath()
{
    ...
    {
        // Normalize quaternion from stored values
        Eigen::Quaterniond q(iter->second[3], iter->second[4], iter->second[5], iter->second[6]);
        q.normalize();
